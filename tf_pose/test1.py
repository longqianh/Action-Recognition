...
# 一些初始化参数
_init_xavier1 = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
_init_xavier = tf.initializers.GlorotUniform()
# _init_norm = tf.truncated_normal_initializer(stddev=0.01)
_init_norm = tf.compat.v1.keras.initializers.TruncatedNormal(stddev=0.01)
# _init_zero = slim.init_ops.zeros_initializer()
_init_zero = tf.compat.v1.zeros_initializer()
_l2_regularizer_00004 = tf.keras.regularizers.l2(0.00004)  # l2正则化
...
class BaseNetwork(object):
        ...
        for op_name, param_dict in data_dict.items():
            if isinstance(data_dict[op_name], np.ndarray):
                ...
                with tf.compat.v1.variable_scope('', reuse=True):
                    ...
                    try:
                        session.run(var.assign(data_dict[op_name]))  # 此处要改
    ...
    def make_var(self, name, shape, trainable=True):
        ...
        return tf.compat.v1.get_variable(name, shape, trainable=self.trainable & trainable, initializer=tf.keras.layers.xavier_initializer())
    ...
    def conv(self,
        ...
        with tf.compat.v1.variable_scope(name) as scope:
            kernel = self.make_var('weights', shape=[
                                   k_h, k_w, c_i / group, c_o], trainable=self.trainable & trainable)
            if group == 1:
    ...
    def se_block(self, input_feature, name, ratio=8):
        ...
        kernel_initializer = tf.keras.layers.variance_scaling_initializer()
